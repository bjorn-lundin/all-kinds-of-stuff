...
package body  SDL2.TTF is
  ...
  procedure Open (Self : in out Font; Filename : in String; Point_Size : in Integer) is    
   --TTF_Font *TTF_OpenFont( const char *file, int ptsize)
  begin
     Self.Ptr :=  SDL2.TTF.Thin.Open(Filename, Point_Size);
  end Open;
  ...
  begin
    if  SDL2.TTF.Thin.Was_Init = 0 then
      return False;
    else 
      return True;
    end if;  
  ...
  begin
     SDL2.TTF.Thin.Set_Font_Style(Self.Ptr, Local_Style);
  end Set_Style;
  ...
  function Get_Styles(Self : in out Font) return Style_Array_Type is
    All_Styles_Numeric : Unsigned_8 := Unsigned_8(  SDL2.TTF.Thin.Get_Font_Style(Self.Ptr));
...
--  begin
--    return  SDL2.TTF.Thin.Get_Font_Style(Self.Ptr);
--  end Get_Style;
  
  --------------------------------------------------------------------------------
  procedure Debug_Print_Style(Self : in out Font)is
  begin
     SDL2.Log.Put_Debug ("Font Style " &  SDL2.TTF.Thin.Get_Font_Style(Self.Ptr)'Img);
  ...
  begin
    return  SDL2.TTF.Thin.Get_Font_Height(Self.Ptr);
  end Get_Height;
  --------------------------------------------------------------------------------
  ...
  begin
    Result :=  SDL2.TTF.Thin.Get_Text_Size(Font_Ptr => Self.Ptr,
                                         Text     => C_Text,
                                         W        => W,
                                         H        => H);
