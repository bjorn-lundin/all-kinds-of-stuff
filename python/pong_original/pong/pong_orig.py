...
# Optionally log to tensorboard if dependency met
try:
  from torch.utils.tensorboard import SummaryWriter
  # Writer will output to ./runs/ directory by default
  writer = SummaryWriter(comment=description)
  print('Logging to tensorboard enabled')
...
resume = args.resume # resume from previous checkpoint?
render = args.render
#render = False
#demo = args.demo
demo = False
...
if resume or demo:
  checkpoint = 'save.p' if not args.demo else 'best.p'
  print('Resuming from checkpoint. Loading:',checkpoint)
...
#  print('type J ', type(J))
#bnl it is a tuple with vector as loc 0
  if type(J) is tuple : 
    I = J[0]
  else:
    I = J  
...
#bnl  return I.astype(np.float).ravel()
...
episode_number = 0
while True:
# in constructor  if render or demo: env.render()

  # preprocess the observation, set input to network to be difference image
  cur_x = prepro(observation)
  #print(type(cur_x),type(prev_x))
...
#  x = cur_x - prev_x if prev_x is not None else np.zeros(D)
  ...
  if done: # an episode finished
    ...
    if not demo and episode_number % 100 == 0: pickle.dump(model, open('save.p', 'wb'))
    reward_sum = 0
